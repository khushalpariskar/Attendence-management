...
def login():
    ...
    def call():
        ...
        if (user_type.get()=="STUDENT"):
            ...
            passwd=Entry(lgn1_frm,textvariable=password)
            passwd.place(x=200,y=160,width=250,height=30)

            Button(lgn1_frm,text='PROCEED',command=verify_std).place(x=185,y=200,width=280,height=50)

        elif(user_type.get()=="TEACHER"):
            def verify_tch():
                num=str(emp_id.get())
                pd=str(password.get())


                sql_cnd="select * from teacher where regno=('"+num+"') and password=('"+ pd+"')"
                mycursor.execute(sql_cnd)
                data = mycursor.fetchall()
                if data==[]:
                    messagebox.askretrycancel(title='ERROR', message='WRONG ID OR PASSWORD')
                    ...
                    messagebox.askokcancel(title='SUCCESSFUL', message="WELCOME! "+num)
                    f = open('regno_password_tch.txt', 'w')
                    f.write(num)
                    import selection_page_tch
            Label(lgn1_frm,text='EMPLOY ID').place(x=100,y=20,width=450,height=30)
            emp_id=Entry(lgn1_frm)
            emp_id.place(x=200,y=60,width=250,height=30)

            Label(lgn1_frm,text='PASSWORD').place(x=100,y=120,width=450,height=30)
            password=Entry(lgn1_frm)
            password.place(x=200,y=160,width=250,height=30)

            Button(lgn1_frm,text='PROCEED',command=verify_tch).place(x=185,y=200,width=280,height=50)
            """def fun():
                root.destroy()
                import starting_page
            Button(lgn1_frm,text='PROCEED',command=lambda :[selection_page(),fun]).place(x=185,y=200,width=280,height=50)
            """
        elif(user_type.get()=="ADMINISTRATION"):
            def verify_tch():
                num=str(emp_id.get())
                pd=str(password.get())


                sql_cnd="select * from admin where regno=('"+num+"') and password=('"+ pd+"')"
                mycursor.execute(sql_cnd)
                data = mycursor.fetchall()
                if data==[]:
                    messagebox.askretrycancel(title='ERROR', message='WRONG ID OR PASSWORD')
...
#-------------------------corresponding address-----------
    ...
    password=Entry(manage_frame,font=('calibri',13),textvariable=password1)
    ...
    def acd_details():
      ...
      #  Label(acd_frame, text="PERSONAL INFORMATION", bg='light blue', fg='black', font=('calibri', 13)).grid(row=0,column=50,padx=0)
        ...
        course=StringVar()
        Label(acd_frame, text="")
        lbl = Label(acd_frame, text="SELECT COURSE:-", font=('calibri', 9),textvariable=course,width=20)
        lbl.grid(row=8, column=0, pady=20, padx=20, sticky="w")
