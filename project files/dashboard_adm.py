...
Label(root,text='admin DASHBOARD', bg='GREEN',width="300",height="2",font=('calibri',13)).pack()
...
Label(root,text="admin'S DASHBOARD",bg='white',padx=10,font=('calibri',13)).place(x=605,y=15)
"""
...
f=open('regno_pswd_tch.txt','r')
data=f.read()
sql_name="select name from admin where regno="+data
mycursor.execute(sql_name)
data1 = mycursor.fetchone()
sql_roll="select regno from admin where regno="+data
sql_sess="select date_of_birth from admin where regno="+data
sql_courses="select courses from admin where regno="+data
...
Label(main_first,text='WELOCME '+str(data1),padx=20,pady=20,font=('calibri',9)).place(x=150,y=40,width=150,height=60)
...
photo=Image.open('profile_tch.jpg')
...
Label(tip_frm,text='QUOTE',padx=20,pady=20,font=('calibri',9,'bold')).place(x=00,y=0,width=350,height=50)
Label(tip_frm,text=qte,padx=20,pady=20,font=('calibri',9,'bold')).place(x=20,y=60,width=300,height=80)
...
def crs_atd():
    classes = Frame(root, bg='green', relief=GROOVE)
    classes.place(height=280, width=650, x=550, y=120)
    Label(classes, text='coursesS AND ATTENDENCE', padx=20, pady=20, font=('calibri', 13, 'bold')).pack()

def tdy_cls():
    classes = Frame(root, bg='green', relief=GROOVE)
    classes.place(height=280, width=650, x=550, y=120)

    sql_cmd_date="select time from monday_tch"
    mycursor.execute(sql_cmd_date)
    d_time=mycursor.fetchall()
    num=len(d_time)
    sql_cmd_cls="select section from monday_tch"
    ...
    for i in range(num):
        ...
        Label(classes,text=p,padx=10,pady=10, font=('calibri', 10, 'bold')).place(height=25,width=70,x=10,y=50*i)
...
Button(root,text="MY coursesS AND ATTENDENCE",padx=50,pady=30,bg='white',font=('calibri',9,'bold'),command=crs_atd).place(x=670,y=91,width=180,height=30)
